...
for n,page in enumerate(doc,1):
    top,bottom=56.16,page.rect.height-56.16;left,right=44.64,page.rect.width-44.64;h=bottom-top;spans=[];texts=[];images=0;blocks=[]
    for block in page.get_text('dict')['blocks']:
        if block['type']==1:
            r=fitz.Rect(block['bbox']);images+=1
            if r.x1>left and r.x0<right and r.y1>top and r.y0<bottom:
                spans.append((max(top,r.y0),min(bottom,r.y1)))
                blocks.append({'kind':'image','bbox':list(r)})
        else:
            for line in block.get('lines',[]):
                ...
                if r.y1>top and r.y0<bottom and r.x1>left and r.x0<right and text.strip():
                    ...
                    blocks.append({'kind':'text','bbox':list(r),'text':text})
    spans.sort();merged=[]
    for y0,y1 in spans:
    ...
    # Preserve exact physical evidence for the publisher's neighbouring-page
    # review. These are printed lines (including captions) and image rectangles,
    # not reconstructed prose or a claim that a proposed figure will fit.
    geometry={'body_bounds_points':[left,top,right,bottom],
              'last_occupied_y_points':merged[-1][1] if merged else None,
              'trailing_space_points':bottom-merged[-1][1] if merged else h,
              'blocks':sorted(blocks,key=lambda b:(b['bbox'][1],b['bbox'][0]))}
    rows.append({'page':n,'unused':unused,'head':round(head,3),'tail':round(tail,3),'largest_internal_gap':round(max(internal or [0]),3),'images':images,'kind':kind,'over30':unused>.30,'candidate_reason':candidate,'review_status':'needs visual review' if unused>.30 else 'within threshold','text_start':' '.join(texts)[:240],'text_end':' '.join(texts)[-160:],'layout_geometry':geometry})

...
const base=process.argv[2]||'http://127.0.0.1:8841/';if(!['localhost','127.0.0.1'].includes(new URL(base).hostname))throw Error('Local review only');
const out='output/playwright/site-public';mkdirSync(out,{recursive:true});const b=await chromium.launch();const rows=[];
try{for(const [host,name,width] of [['caithrin.com','caithrin',390],['manifund.substack.com','The Fox Says',1280]]){
 ...
 await p.goto(base);await p.screenshot({path:`${out}/${width}-hero.png`});
 ...
 await p.waitForFunction(()=>!document.querySelector('#sample-mode').disabled);await p.screenshot({path:`${out}/${width}-sample.png`});const source=await p.locator('#sample-source').getAttribute('href'),count=await p.locator('#sample-position').textContent();
 await p.locator('#sample-enlarge').click();await p.locator('#sample-mode').click();await p.locator('#sample-scale').click();await p.screenshot({path:`${out}/${width}-actual-size.png`});await p.keyboard.press('Escape');
 if(errors.length)throw Error(errors.join(';'));rows.push({host,publication:name,source,view:count,real_public_reads:true,production_writes:false});console.log('PASS',name,source);await p.close();

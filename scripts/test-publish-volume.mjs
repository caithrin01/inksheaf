...
async function scenario({alwaysRepair=false,changeSource=false,visionFails=false,hold=false,leading=.66,interrupted=false}={}){
  const dir=mkdtempSync(join(tmpdir(),'publisher-volume-')),pdf=join(dir,'book.pdf');
  const doc=await PDFDocument.create();doc.addPage([432,648]);doc.addPage([432,648]);writeFileSync(pdf,await doc.save());
  ...
  const emit=async e=>events.push(e),publisher={emit,vision:async()=>{if(visionFails)throw Error('provider unavailable');return{text:'[]'};},layout:async input=>{
    const result={decisions:input.pages.map(p=>({page:p.page,decision:hold?'needs_review':'repair',candidate_id:hold?null:input.candidates.find(c=>c.page===p.page)?.id,reason:hold?'A content defect needs investigation.':'Bring the stranded ending back using measured leading.'}))};

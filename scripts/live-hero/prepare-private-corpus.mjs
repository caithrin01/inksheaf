...
for(const row of picked.selected){
 ...
 const record={host:row.host,category:row.category,status:'reading'};result.publications.push(record);save();
 ...
 try{
  ...
  const outcomes=await mapConcurrent(posts,async post=>{
   ...
   }catch(e){return {error:{id:post.id,slug:post.slug,error:String(e.message).slice(0,200)}};}
  },{concurrency:4});
  ...
  record.fetched_bodies=full.length;record.failures=errors.length;record.status=errors.length?'incomplete source capture':'sources complete';

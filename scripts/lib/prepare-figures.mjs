...
For a clearly regular montage with four to six columns of thumbnails and one to four rows, supply its grid column and row counts. Otherwise grid is null. Supply a grid only for reading/small_text images whose individual titles need reading, never for a chart, table or ordinary photograph. This permits enlarged details alongside the complete original.
...
export async function prepareSourceFigures({html,baseDir,directory,ask,onResult=async()=>{}}){
  ...
  await mapConcurrent(batches,async(batch,index)=>{
    ...
    for(const answer of result.figures){const source=batch.find(f=>f.id===answer.figure_id);roles[source.id]={...answer,image_sha256:source.image_sha256,...(answer.grid?{detail_panels:prepareFigureDetails(source,answer.grid,join(directory,'details'))}:{})};}

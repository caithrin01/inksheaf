...
for(const name of ['async-work.mjs','publisher-session.mjs','publisher-layout.mjs','publish-volume.mjs','figure-role.mjs','figure-confirmation.mjs','prepare-figures.mjs','figure-details.mjs','notes-detect.mjs','layout-evidence.mjs','glyph-evidence.mjs','artifact-evidence.mjs','page-text-evidence.mjs','paragraph-boundaries.mjs','running-matter.mjs','page-review.mjs','fit.mjs','prepared-typesetting.mjs','source-request.mjs','copy-fit.mjs','typst-emit.mjs'])REVIEW_POLICY.update(readFileSync(new URL(name,import.meta.url)));
...
The following-image fit fields quantify the constraint: minimum_height_shortfall_points and minimum_linear_reduction_fraction_to_fit exclude caption/spacing, so they are lower bounds. If readable text requires the protected size and the image must follow this prose, a break before it can have a specific figure_sequence purpose even with a large preceding gap. Inspect the actual sequence and name its content and constraint. The absence of a repair candidate by itself proves neither a defect nor good composition; a needs_review reason must identify what remains wrong or uncertain beyond that absence. Never accept unreadable detail, a stranded prose fragment or an interrupted paragraph on this basis.

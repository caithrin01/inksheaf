...
import {FigureConfirmation,ReadingOrderConfirmation} from './figure-confirmation.mjs';
...
30% unused body space triggers review; it is not a maximum permitted gap. Every page above it needs an applicable repair or a specific design reason.
...
source_observations records completed source-image comparisons for this exact PDF page. They apply only to the specified glyph or markup observation. If the printed screenshot faithfully preserves lettering or literal markup from the source, do not call that same pictured content a new typesetting failure. Optional artifact_evidence distinguishes actual PDF text objects from lettering inside an exactly matched embedded source bitmap. Still inspect the space around it and any separate loss, scale, crop or content defect; source fidelity never approves a page's composition.
${TEXT_EVIDENCE_TASK} These records never approve a page's composition.
following_source_figure identifies the exact next image, its independently inspected source role, its current reading size, and the space before it. reading_size_protected means the renderer currently protects this reading setting; it is not proof that every smaller measured setting would be unreadable. The image must stay in source order. Judge whether breaking before that named image has a specific reading purpose. The image belongs to this decision even when the target page contains only text. Do not report no following image when this record and the next-page raster show one. This evidence never excuses a content defect or automatically approves the composition.
The following-image fit fields quantify the constraint: minimum_height_shortfall_points and minimum_linear_reduction_fraction_to_fit exclude caption/spacing, so they are lower bounds. If readable text requires the protected size and the image must follow this prose, a break before it can have a specific figure_sequence purpose even with a large preceding gap. letter_points_if_fitted_to_gap below letter_floor_points is measured evidence that shrinking the image into the gap would make its source lettering unreadable. Inspect the actual sequence and name its content and constraint. The absence of a repair candidate by itself proves neither a defect nor good composition; a needs_review reason must identify what remains wrong or uncertain beyond that absence. Never accept unreadable detail, a stranded prose fragment or an interrupted paragraph on this basis.
A fit_figure candidate names the page with the gap in candidate.page and the image's current location in candidate.figure_page. It fits that image into the preceding gap; return the repair for candidate.page. picture_evidence records a separate inspection of that exact source image, identified by its image hash. Such candidates are supplied for confirmed photographs/illustrations, and for images with text to read only when their measured lettering still prints at or above letter_floor_points after fitting (letter_points_after_fit); unknown roles have no shrinking candidate. Judge whether the proposed picture size suits the page; do not reject it merely because the full-size image currently prints on the following page. Source order and pixels are preserved and the resulting PDF still receives full review.
...
export async function publisherSession({directory, env=process.env, fetchImpl=fetch, checkpointStore}) {
  ...
  const vision = async ({model,images,text,maxTokens,check}) => {
    ...
    try{({result:answer,usage}=await ask.withUsage(request));}catch(error){
      ...
      ({result:answer,usage}=await ask.withUsage({...request,task:task+' The previous response failed schema validation: '+error.message+'. Recheck the specific defect and return a valid answer with a concise note under 200 characters.'}));
    }
    const result=role==='reader'?answer.findings:answer;const latest=new Map(state.cache);latest.set(key,result);state.cache=[...latest];await save();
    return {text:JSON.stringify(result),usage};
  };
  const layoutBatch=async (input,imagesByPage)=>{
    ...
    const request={model:PUBLISHER_MODELS.publisher.id,task:LAYOUT_TASK,schema:LayoutReviewDecisions,input,imageHashes,maxTokens};
  ...
  const layout=async (input,{imagesByPage}={})=>{
    ...
    for(const result of await mapConcurrent(layoutBatches(input,imagesByPage?3:6),batch=>layoutBatch(batch,imagesByPage)))decisions.push(...result.decisions);
  ...
  return {emit,vision,layout,figureRole,sourceFigures,sourceNotes,selection,
    ...
    return publishSelection({posts,publication,identity,overrides:{...overrides,...restored},cache,ask,emit:e=>emit({...e,volume}),saveCache:async c=>{state.cache=[...new Map([...state.cache,...c])];await save();}});

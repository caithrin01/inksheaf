...
export async function publisherSession({directory, env=process.env, fetchImpl=fetch}) {
  ...
  const run=String(env.GITHUB_RUN_ID||'local')+'.'+String(env.GITHUB_RUN_ATTEMPT||1);
  const remote=Boolean(id&&site&&secret);
  const sign=m=>createHmac('sha256',secret).update(m).digest('hex');
  let state={journal:{calls:[],spent:0},cache:[],runs:{}}, revision=0;
  ...
  if(remote){const saved=await request(`/api/publisher-state?id=${id}&sig=${sign(`publisher-state:${id}`)}`);state=saved.state||state;revision=saved.revision;}
  ...
  const vision = async ({model,images,text,maxTokens}) => {
    ...
    const ask=openRouterPublisher({key:env.OPENROUTER_API_KEY,journal:state.journal,persist:save});
  ...
  return {emit,vision,layout,read:async({posts,publication,identity,overrides,volume})=>{
    const cache=new Map(state.cache);
    const ask=openRouterPublisher({key:env.OPENROUTER_API_KEY,journal:state.journal,persist:save});
    return publishSelection({posts,publication,identity,overrides,cache,ask,emit:e=>emit({...e,volume}),saveCache:async c=>{state.cache=[...c];await save();}});

...
for(const file of ['fit.mjs','prepared-typesetting.mjs','source-request.mjs','copy-fit.mjs','figure-details.mjs','prepare-figures.mjs','notes-detect.mjs','typst-emit.mjs','../build-book.mjs','../render-book.sh','../pdf-whitespace-audit.py','../blank-measure.py'])renderer.update(readFileSync(new URL(file,import.meta.url)));
...
export async function saveRenderCheckpoint({book,directory,scope,identity,store}){
  ...
  report.fit=Object.fromEntries(['pass','defer','fitFigs','fitText','backLinks','inFlow','readingFigures','pictureFigures','spacing_requires_review'].filter(k=>report.fit[k]!==undefined).map(k=>[k,report.fit[k]]));

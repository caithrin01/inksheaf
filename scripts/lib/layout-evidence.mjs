...
import {contactSheetGroups,pageOf} from './page-review.mjs';
...
export function prepareLayoutEvidence(input,{directory,rasterDirectory,pageCount,policy}){
  ...
  const neighbours=input.pages.map(page=>{
  ...
  const sheets=contactSheetGroups(neighbours.map(pages=>pages.map(p=>files.get(p))),join(directory,'layout-images',input.pdf_hash),{format:'png'});
  const packet={...input,pages:input.pages.map((page,index)=>{
    const pages=neighbours[index],sheet=sheets[index];

...
await journey('sample-keyboard-and-status',{},async page=>{
 // A visible iframe can still be about:blank while its document is in flight.
 await page.route('**/reader/**',async route=>{await new Promise(resolve=>setTimeout(resolve,350));await route.continue();});
 await preview(page);
 await page.locator('#bookwrap').focus();await page.keyboard.press('Enter');
 await page.locator('#sample-frame').waitFor({state:'visible',timeout:20000});
 const frame=page.frameLocator('#sample-frame');
 await frame.locator('#reading .artbody p').first().waitFor({state:'visible',timeout:20000});
 assert.equal(await frame.locator('html').evaluate(()=>location.pathname),'/reader/','reader frame loads the intended document');
 await page.waitForFunction(()=>document.querySelector('#sample-status').textContent==='',null,{timeout:3000});
 assert.equal(await frame.locator('#status').textContent(),'','reader loading message clears when text is visible');
 await page.locator('#view-cover').click();assert.equal(await page.locator('#view-cover').getAttribute('aria-pressed'),'true');
});

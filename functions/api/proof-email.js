...
export async function onRequest({ request, env }) {
  ...
  try {
    const version = await env.DB.prepare(`SELECT v.id, v.signup_id, v.status, s.email, s.email_verified_at
    ...
    if (latest?.id !== id || version.status !== 'proofed') return json({ ok: false, error: 'this edition is no longer awaiting proof review' }, 409);
    ...
    const result = await sendQueuedEmail(env, row.id);

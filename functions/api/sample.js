...
export async function onRequest({request,env}) {
  ...
  const candidates=(preview.sample||[]).filter(p=>/^[a-z0-9][a-z0-9-]{0,199}$/i.test(p.slug||'')).slice(0,2);
  ...
  for(const candidate of candidates) {
    ...
    try {
      ...
      for(let hop=0;hop<3;hop++){
        ...
        if(next.protocol!=='https:'||next.username||next.password||next.port||next.hostname.replace(/^www\./,'')!==host.replace(/^www\./,'')||next.pathname!==`/api/v1/posts/${candidate.slug}`)break;
        target=next.href;
      }
      if(!r.ok || Number(r.headers.get('content-length')||0)>2_000_000)continue;
      const reader=r.body.getReader();let bytes=0;const chunks=[];
      for(;;){const {value,done}=await reader.read();if(done)break;bytes+=value.byteLength;if(bytes>2_000_000){await reader.cancel();throw new Error('body limit');}chunks.push(value);}
      const all=new Uint8Array(bytes);let offset=0;for(const chunk of chunks){all.set(chunk,offset);offset+=chunk.length;}
      const post=JSON.parse(new TextDecoder().decode(all));

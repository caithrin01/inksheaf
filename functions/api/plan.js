...
import { fetchArchive, parseHost } from "./preview.js";
...
export async function onRequest({ request, env }) {
  ...
  if (pay && pay.summary_version === 6 && pay.editorial && pay.editorial.planned_by === "editor")
  ...
  const t0 = Date.now();
  const editorial = await planEdition({ posts, identity: result.identity, host, capped: !!result.data?.capped, apiKey: env.ANTHROPIC_API_KEY, openrouterKey: env.OPENROUTER_API_KEY });

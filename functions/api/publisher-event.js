...
export async function onRequest({request, env}) {
  ...
  try {
    await env.DB.prepare('INSERT INTO publisher_events (signup_id,run_id,sequence,kind,payload) VALUES (?,?,?,?,?) ON CONFLICT(signup_id,run_id,sequence) DO NOTHING').bind(id,run,sequence,b.event.kind,payload).run();
